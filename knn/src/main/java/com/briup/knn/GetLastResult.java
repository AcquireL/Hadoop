...
/*
 * Knn第六步：
 * 	  根据标签统计个数，个数最多的则为识别出来的结果，如果有个数相同的标签，则计算平均相似度
 * 		输入位置：hdfs文件系统：/knn_data/gtk_result/part-r-00000
 * 		输出位置：hdfs文件系统：/knn_data/last_result
 */
...
public class GetLastResult 
	...
	public int run(String[] arg0) throws Exception {
		...
		Job job = Job.getInstance(conf, "glr");
	...
	public static class GLRReducer 
		...
		protected void reduce(Text key, 
			Iterable<TagAvgNum> values,
				Reducer<Text, TagAvgNum, 
				Text, NullWritable>.Context context) throws IOException, InterruptedException {
			Iterator<TagAvgNum> ite = 
				values.iterator();
			TagAvgNum max = 
				new TagAvgNum(ite.next());
			while(ite.hasNext()) {
				TagAvgNum current = 
					new TagAvgNum(ite.next());
				if(max.getNum().get() < 
